...
export default function CfbTop25Card() {
    ...
    return (
      <article className="card">
        ...
        <p className="mt-2 text-sm text-zinc-700">
          {error} (This might be due to missing or out-of-date ranking data.)
        </p>
      </article>
    );
  }
  ...
  const top10: CfbTeamRank[] = data.top25.slice(0, 10);

  return (
    <article className="card">
      <h2 className="text-xl font-semibold">College Football Rankings</h2>
      <p className="mt-1 text-xs text-zinc-500">
        {data.year} season · Updated {data.lastUpdated}
      </p>

      <p className="mt-3 text-sm text-zinc-700">{data.methodSummary}</p>
